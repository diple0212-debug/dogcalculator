...
const POST_CONTENTS: Record<string, React.ReactNode> = {
  '15': (
    <div className="space-y-8">
      ...
      <p>강아지를 처음 키우는 초보 보호자부터 베테랑 집사까지 평생을 고민하는 숙제가 바로 '사료 선택'입니다. 반려동물 용품점에 가면 산더미처럼 쌓인 알갱이 사료(건식)와 화려한 캔/파우치 제품(습식) 사이에서 결정 장애가 오기 마련이죠. 2026년 수의학 트렌드는 단순히 '무엇이 더 좋다'가 아닌, **'개체별 건강 상태에 따른 맞춤 선택'**을 강조합니다. 오늘 똑똑한 집사에서는 건식과 습식 사료의 장단점을 과학적으로 분석하고, 전문가들이 제안하는 최적의 급여 솔루션을 정리해 드립니다.</p>

      <h2 className="text-2xl font-black text-gray-800">1. 건식 사료(Kibble): 경제성과 구강 건강의 강자</h2>
      <p>우리가 흔히 말하는 '개사료'는 대부분 건식입니다. 수분 함량이 10% 내외로 낮아 보관이 용이하고 가격이 합리적이라는 특징이 있습니다.</p>
      <ul className="list-disc pl-5 space-y-4">
        <li><strong>치석 예방 효과:</strong> 딱딱한 알갱이를 씹는 과정에서 치아 표면의 치태를 물리적으로 긁어내는 연마 작용을 합니다. 양치질을 싫어하는 아이들에게는 구강 관리에 도움이 됩니다.</li>
        <li><strong>보관과 급여의 편리함:</strong> 실온 보관이 가능하고 사료 그릇에 오래 두어도 상할 위험이 적습니다. 자동 급식기를 사용하는 보호자에게는 필수적인 선택입니다.</li>
        <li><strong>높은 에너지 밀도:</strong> 부피 대비 칼로리가 높아 적은 양으로도 필요한 에너지를 충분히 공급할 수 있습니다. 성장기 퍼피나 활동량이 많은 강아지에게 적합합니다.</li>
      </ul>

      <h2 className="text-2xl font-black text-gray-800">2. 습식 사료(Wet Food): 기호성과 수분 공급의 왕</h2>
      <p>캔이나 파우치에 들어있는 습식 사료는 수분 함량이 70~80%에 달합니다. 마치 사람이 갓 요리한 음식을 먹는 것과 같은 경험을 제공합니다.</p>
      <ul className="list-disc pl-5 space-y-4">
        <li><strong>압도적인 기호성:</strong> 풍부한 고기 향과 부드러운 식감 덕분에 입맛이 까다로운 강아지나 아픈 후 회복 중인 아이들에게 최고의 식사가 됩니다.</li>
        <li><strong>자연스러운 수분 섭취:</strong> 물을 잘 마시지 않는 강아지들에게 매우 중요합니다. 충분한 수분 공급은 결석 예방과 신장 건강 유지에 결정적인 역할을 합니다.</li>
        <li><strong>체중 조절에 유리:</strong> 수분이 많아 부피 대비 칼로리가 낮습니다. 많이 먹어도 살이 덜 찌기 때문에 비만 관리 중인 강아지에게 포만감을 주기에 좋습니다.</li>
        <li><strong>노령견에게 적합:</strong> 치아가 약하거나 잇몸 질환이 있는 노령견들이 통증 없이 편안하게 영양을 섭취할 수 있게 도와줍니다.</li>
      </ul>

      <div className="bg-orange-50 p-6 rounded-3xl border-l-4 border-orange-400 shadow-sm text-gray-700 my-6">
        <h4 className="font-bold text-orange-800 mb-2">💡 수의학 전문가의 한마디</h4>
        <p className="text-sm m-0 leading-relaxed">
          "건식은 '관리'의 사료고, 습식은 '케어'의 사료입니다. 하지만 가장 좋은 것은 두 사료의 장점을 합친 <strong>혼합 급여(Mixed Feeding)</strong>입니다. 아침에는 바쁜 보호자를 위해 건식을, 저녁에는 아이의 음수량과 즐거움을 위해 습식을 섞어주는 방식을 추천합니다."
        </p>
      </div>

      <h2 className="text-2xl font-black text-gray-800">3. 상황별 추천 사료 선택 가이드</h2>
      <p>우리 아이의 현재 상태에 따라 정답은 달라집니다. 아래 체크리스트를 확인해 보세요.</p>
      <ul className="list-disc pl-5 space-y-3">
        <li><strong>슬개골 탈구/비만 관리 중이라면:</strong> 습식 사료 비중을 높여 칼로리를 제한하고 다이어트 효과를 극대화하세요.</li>
        <li><strong>음수량이 부족해 소변 색이 진하다면:</strong> 무조건 습식 사료를 병행하거나 건식 사료를 물에 불려 주어야 합니다.</li>
        <li><strong>입 냄새가 심하고 치석이 잘 생긴다면:</strong> 건식 사료 위주로 급여하되, 알갱이 크기가 큰 '덴탈 전용 사료'를 선택하는 것이 유리합니다.</li>
        <li><strong>신장 기능이 저하된 시니어견이라면:</strong> 신장에 무리를 주지 않는 단백질원과 충분한 수분이 포함된 처방 습식 사료가 필수입니다.</li>
      </ul>

      <h2 className="text-2xl font-black text-gray-800">4. 사료 변경 시 반드시 지켜야 할 '7일 법칙'</h2>
      <p>건식에서 습식으로, 혹은 그 반대로 사료를 바꿀 때는 강아지의 장이 적응할 시간이 필요합니다. 첫날은 기존 사료 90%에 새 사료 10%를 섞고, 매일 비율을 10~20%씩 늘려가며 일주일 동안 천천히 교체해야 설사나 구토를 예방할 수 있습니다.</p>

      <h2 className="text-2xl font-black text-gray-800">5. 보호자가 저지르는 흔한 실수</h2>
      <p>습식 사료를 급여할 때 가장 큰 실수는 **'남은 음식을 실온에 방치하는 것'**입니다. 수분이 많아 세균 번식이 매우 빠르므로, 개봉 후 30분 이내에 먹지 않은 것은 바로 폐기하거나 냉장 보관해야 합니다. 또한 습식 사료는 치아 사이에 잘 끼므로, 습식 비중이 높다면 반드시 매일 양치질을 해주어야 합니다.</p>

      <h3 className="text-2xl font-black text-gray-800 mt-10">결론: 정답은 아이의 '변'과 '활력'에 있습니다</h3>
      <p>사료를 바꾼 뒤 아이의 변이 적당히 단단하고 윤기가 흐르는지, 털에 윤기가 생기고 눈동자가 초롱초롱해졌는지 관찰해 보세요. 어떤 종류의 사료든 우리 아이가 즐겁게 먹고 건강한 지표를 보여준다면 그것이 바로 최고의 사료입니다. 똑똑한 집사의 사료량 계산기로 적정 칼로리를 체크하고, 아이에게 맞는 최적의 식단을 선물해 주세요. 당신의 정성이 아이의 수명을 늘립니다.</p>
    </div>
  ),
  '14': (
    <div className="space-y-8">
